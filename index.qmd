...
gapminder |> filter(year == 2007) |> ggplot(aes(x = gdpPercap, y = lifeExp, colour = continent)) + geom_point() + geom_smooth(method = "lm") + ggtitle("reLacion entre el pib per capita y esperanza de vida en 2007") + xlab("PIB per capita(EEUU $)") + ylab ("Esperanza de vida (Años)") + labs(color = "continentes")
```
